...
describe("verticalPara", () => {
  ...
  it("lo desconocido cae al genérico (saludo_dueno_v2)", () => {
    expect(verticalPara("manufacturer").plantilla).toBe("saludo_dueno_v2");
...
describe("verticalPorSlug", () => {
  it("encuentra el vertical por su slug (el que eligió el humano en la UI)", () => {
    ...
    expect(verticalPorSlug("generico").plantilla).toBe("saludo_dueno_v2");
  });

...
Y esa suposición **es la misma sobre la que se apoya la tabla**: hoy este panel es el único consumidor de Places del proyecto, así que la cuenta propia se acerca todo lo que se puede sin preguntarle a Google. Sigue siendo un piso —hay caminos que facturan sin dejar fila, ver Riesgo 1—, pero Monitoring daría hoy una cifra muy parecida a cambio de una cuenta de servicio, credenciales nuevas, una API más que puede caerse y una decisión incómoda sobre qué hace el botón cuando se cae.
...
- El campo compara **en centavos, no en bytes** (`coincideMonto`, en `src/lib/admin/monto.ts`): `42,00`, `42.00`, `US$ 42,00` y `42` valen lo mismo, porque el teclado numérico de muchos celulares no trae coma y un peaje que no se puede pagar desde el celular no protege, bloquea. Ni la palabra "gastar" ni un texto fijo: **el número**. *(Corregido en la ronda 3 de la rama: la versión anterior comparaba la cadena tal cual y rechazaba el punto.)*
...
**El tope se cobra antes de despachar, no después de renderizar.** El techo de llamadas del permiso viaja al hook del barrido como presupuesto y cada worker pide permiso en el mismo tick síncrono en que despacha (`presupuestoDeEmisiones`). Mirarlo en un efecto de React dejaba pasar las cuatro llamadas que ya iban en vuelo — y con la cuota gratis como techo, esas eran justo las primeras que Google cobraba, sobre un botón que decía "gratis". Ahora se emiten exactamente `tope` llamadas. *(Ronda 3.)*

**Continuar tras el tope.** El tramo de la continuación es **lo que falta en la cola**, todo de pago (al frenarse, la línea del gratis ya quedó atrás): el botón nombra ese costo y el techo lleva el margen del 2× para la subdivisión, como cualquier tanda de pago. Pide el monto escrito cuando el tramo se paga **más de lo que la persona ya confirmó tecleando** — el mayor monto que haya escrito, en el diálogo o en un Continuar anterior. El margen del techo no cuenta como confirmado: aprobar US$ 17,50 no confirma US$ 35. Así el que arrancó gratis teclea (no confirmó nada), el que arrancó pagando sigue a un clic mientras la cola no crezca por encima de lo que aprobó, y el mixto teclea. *(Ronda 3: antes cada Continuar concedía el `tope` del permiso inicial —la cuota entera— y el margen contaba como confirmado, así que un barrido que arrancó pagando podía seguir a un clic sin límite.)*
...
0. **Deploy antes del SQL.** Si el código nuevo sube con la base en la firma de cuatro argumentos, cada tesela se cobra y ninguna se anota. El handler reconoce ese caso (`PGRST202`, la firma del RPC no existe → `esErrorDeMigracion`) y responde `sin_migracion`; el cliente lo trata como mortal y frena a la primera tesela, no a la 310. Además, en los caminos en que Google cobró pero la tesela no se pudo anotar (cuerpo ilegible, upsert caído, RPC caído) el handler escribe igual la fila de `consultas_places`, para que el conteo del mes no se quede corto justo en los fallos. *(Ronda 3.)*
1. **El conteo ya es un piso, no la verdad exacta.** *(Corregido en la revisión final de la rama: esta spec afirmaba que hoy es exacto y se degradaría mañana. No es cierto y no puede prometerse.)* Aunque este panel sea el único consumidor de Places del proyecto, la cuenta se queda corta por dos vías que existen desde el primer día: las llamadas que Google cobró pero cuya fila no se llegó a escribir —cuerpo ilegible, fallo del upsert, fallo de `anotar_tesela`, timeout de plataforma— y el corte por SKU (`consultasDelMes` cuenta solo `origen = 'barrido'`, porque `searchNearby` y `searchText` son SKUs con cuotas separadas; si Google algún día las agrupara, el número se quedaría corto por el otro origen). Y se degrada más en cuanto otra cosa use la key — y nadie recibe un aviso cuando eso pasa. El riesgo concreto no es equivocarse por poco: es que alguien nuevo lea "te quedan 800 gratis", gaste creyendo que no cuesta, y el contador que existe para protegerlo lo haya engañado. Mitigado diciendo en pantalla que la cifra es "según lo que este panel lleva registrado" y dejando el enlace a las métricas de Google como fuente de verdad. Tampoco ve el consumo anterior a esta migración.
